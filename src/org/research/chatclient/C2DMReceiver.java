...
public class C2DMReceiver extends BroadcastReceiver{
	...
	private void handleMessage(Context context, Intent intent)
	{
		...
		PendingIntent pendIntent = PendingIntent.getActivity( context, 0, new Intent(context, BaseActivity.class), PendingIntent.FLAG_CANCEL_CURRENT);
		notification.setLatestEventInfo(context, "Chat Client", "You have a new Message", pendIntent );
		mManager.notify(0, notification);
		Intent i = new Intent();
		i.putExtra(BaseActivity.NOTI_ID, 1);
		i.setAction(BaseActivity.BROADCAST_ACTION);
